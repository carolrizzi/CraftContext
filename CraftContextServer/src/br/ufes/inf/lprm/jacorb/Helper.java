...
public class Helper {
	...
	public static void initChannel (String channelName) {
		try{
			Object obj = Server.poa.servant_to_reference(new EventChannelImpl(Server.orb, Server.poa));
			Server.nc.bind(Server.nc.to_name(channelName), obj);

			System.out.println("Channel " + channelName + " started");
		}catch (Exception e) {
			System.err.println("Could not start the channel " + channelName);
			e.printStackTrace();
		}
	}
